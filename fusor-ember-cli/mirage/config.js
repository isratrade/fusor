...
export default function() {
  ...
  this.post('/fusor/api/openstack/deployments/:id/underclouds', (schema, request) => {
    let od = schema.db.openstackDeployments.find(1);
    od.undercloudAdminPassword = 'undercloudAdminPassword' ;
    od.undercloudIpAddress = '192.168.234.254';
    od.undercloudSshUsername = 'root' ;
    od.undercloudSshPassword = 'vagrant' ;
    return {'undercloud': 2};
  });

  this.get('/fusor/api/openstack/deployments/:id/underclouds/:id', (schema, request) => {
    return {
      'deployed': true,
      'failed': false
    };
