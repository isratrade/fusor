export default function() {
  ...
  this.post('/fusor/api/v21/deployments', function(db, req) {
    let deployment = JSON.parse(req.requestBody).deployment;
    if (Ember.isBlank(pet.name)) {
      let body = { errors: { name: ["can't be blank"] } };
      return new Mirage.Response(422, { some: 'header' }, body);
    } else {
      return { pet: db.deployments.insert(deployment) };
    }
  });

  this.put('/fusor/api/v21/deployments/:id', function(db, req) {
    var id = request.params.id;
    let deployment = JSON.parse(req.requestBody).deployment;
  ...
  this.delete('/fusor/api/v21/deployments/:id', function(db, req) { }, 200);
