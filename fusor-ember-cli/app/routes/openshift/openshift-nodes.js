...
export default Ember.Route.extend({
  ...
  setupController(controller, model) {
    ...
    if (isRhev && !(isOpenStack)) {
      return this.controllerFor('deployment').set('model.openshift_install_loc', 'RHEV');
    } else if (!(isRhev) && isOpenStack) {
      return this.controllerFor('deployment').set('model.openshift_install_loc', 'OpenStack');
    }
  },
